...
from utils import limit_past, kl, entropy, bits2int, int2bits, is_sent_finish, num_same_from_beg,js


def encode_saac(model, enc, message, context, finish_sent=False, device='cpu', temp=1.0, precision=26, topk=50000,
                nucleus=0.95, verbose=False, with_case_study=False):
    ...
    with torch.no_grad():
        ...
        while i < len(message) or (finish_sent and not sent_finish):
            ...
            if i >= len(message):
                ...
                # Debug 输出（核心跟踪）
                print(
                    f"[Encode] k={k}, prob={probs_temp[selection].item():.5f}, ")
                # print(
                #     f"[Encode] k={k}, idx={selection}, token_id={indices[selection].item()}, prob={probs_temp[selection].item():.5f}, "
                #     f"u={message_idx}, cur_range=({cur_interval[0]},{cur_interval[1]}), f={probs_final[selection].item()}, "
                #     f"base={new_int_bottom}, s={cur_interval[1] - cur_interval[0]}, bit_pos={i}, encoded={num_bits_encoded}")
                ...
                # print(cur_kl)
                total_kl += cur_kl
                total_js+= js(q, logq, log_probs[:len(q)])
    ...
    avg_js=total_js/total_num_for_stats
    words_per_bit = total_num_for_stats / i

    return output[len(context):].tolist(), avg_NLL, avg_KL, words_per_bit, avg_Hq, topk_list, case_studies,avg_js


def decode_saac(model, enc, text, context, device='cpu', temp=1.0, precision=16, topk=50000, nucleus=0.95):
