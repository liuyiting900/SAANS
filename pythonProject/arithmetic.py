...
def encode_arithmetic(model, enc, message, context, finish_sent=False, device='cpu', temp=1.0, precision=26,
                      topk=300):
    ...
    def select_safe_index(probs, interval_range, epsilon):
        safe_threshold = int(epsilon * interval_range)
        probs_int = torch.round(probs * interval_range).long()

        # 找到所有大于 ε 的候选
        valid_indices = (probs_int >= safe_threshold).nonzero().view(-1)
    ...
    with torch.no_grad():
        ...
        while i < len(message) or (finish_sent and not sent_finish):
            ...
            if i >= len(message):
                ...
                # print("probs_temp_int:", probs_temp_int)
                ...
                # Debug 输出（核心跟踪）
                print(f"[Encode] k={k}, idx={selection}, token_id={indices[selection].item()}, prob={probs_temp[selection].item():.5f}, "
    ...
    total_time = end_time - start_time  # 计算函数运行时间

    return output[len(context):].tolist(), avg_NLL, avg_KL, words_per_bit, avg_Hq, total_time


def decode_arithmetic(model, enc, text, context, device='cpu', temp=1.0, precision=26, topk=500):
    ...
    with torch.no_grad():
        ...
        while i < len(inp):
            ...
            # Extract renormalized bits (公共前缀)
            num_bits_encoded = num_same_from_beg(new_int_bottom_bits_inc, new_int_top_bits_inc)
